                        ...
                        <div key={index} className="col-md-4 mb-3 d-flex justify-content-center">
                            <div className="card shadow" style={{ width: '20rem' }}>
                                <div style={{ height: '200px', overflow: 'hidden' }}>
                                    <img src={training.trainingPicture} className="card-img-top" alt="Training" style={{ objectFit: 'cover', height: '100%' }} />
                                </div>
                                <div className="card-body">
                                    <h4 className="card-title">{training.title}</h4>
                                    <p className="card-text">{training.description}</p>
                                    <button className="btn btn-success">
                                        <Link to={`/internaltraining2?id=${training._id}`} style={{ textDecoration: 'none', color: 'white' }}>
                                            Apply
                                        </Link>
                                    </button>
                                </div>
